...
@section('service_title', 'تبدیل حساب به شبا')


@section('submit_text', 'تبدیل به شبا')

@section('form_fields')
    @include('front.services.custom.partials.account-field')
    @include('front.services.custom.partials.bank-select-field')
@endsection
...
@section('other_services_section')
    ...
    <div class="mt-8 mb-6">
        <div class="grid grid-cols-2 gap-4">
            ...
            <a href="{{ route('services.show', ['slug1' => 'card-account']) }}" 
               class="flex items-center justify-center p-4 bg-white border border-primary-normal rounded-xl hover:bg-primary-normal hover:text-white transition-all duration-300 group">
                <div class="text-center">
                    <svg class="w-7 h-7 text-primary-normal mx-auto mb-3 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
                    </svg>
                    <span class="text-sm font-semibold text-primary-dark group-hover:text-white transition-colors">کارت به حساب</span>
                </div>
            </a>
...
@section('bank_slider_section')
    @if(isset($banks) && count($banks) > 0)
        @include('front.components.bank-slider', ['banks' => $banks, 'serviceSlug' => 'account-iban'])
    @endif
@endsection 