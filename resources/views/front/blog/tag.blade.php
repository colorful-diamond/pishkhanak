...
<main class="flex flex-col self-center w-full rounded-[32px] max-md:max-w-full max-sm:px-2.5 transition-all duration-300">
    <div class="box-border flex relative flex-col shrink-0 mx-auto w-full max-w-screen-xl transition-all duration-300">
        {{-- Tag Header --}}
        <div class="tag-header w-full mb-8 px-6 py-12 bg-gradient-to-br from-sky-50 to-indigo-50 rounded-3xl">
            <nav class="breadcrumb mb-6">
                <ol class="flex items-center gap-2 text-sm text-neutral-600">
                    ...
                    <li><span class="text-neutral-400">/</span></li>
                    <li class="text-zinc-800 font-medium">برچسب: {{ $tag->name }}</li>
                </ol>
            </nav>
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <h1 class="text-3xl md:text-4xl font-bold text-zinc-800 font-['IRANSansWebFaNum']">
                    {{ $tag->name }}
                </h1>
            </div>
            
            <p class="text-lg text-neutral-600 font-['IRANSansWebFaNum'] mb-6">
                مشاهده تمام مقالات با برچسب "{{ $tag->name }}"
            </p>
            
            <div class="tag-stats flex items-center gap-6">
            ...
            <div class="flex-1">
                    ...
                    <div class="posts-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <article class="post-card bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 group">
                            <div class="thumbnail relative h-48 bg-gradient-to-br from-sky-100 to-indigo-100 overflow-hidden">
                                ...
                                <div class="absolute bottom-3 right-3 flex flex-wrap gap-1">
                                    @foreach($post->tags->where('id', '!=', $tag->id)->take(2) as $otherTag)
                                    <a href="{{ route('app.blog.tag', $otherTag->slug) }}" 
                                       class="px-2 py-1 bg-white/90 backdrop-blur-sm text-neutral-700 text-xs font-medium font-['IRANSansWebFaNum'] rounded hover:bg-white transition-all duration-300">
                                        {{ $otherTag->name }}
                                    </a>
                                    @endforeach
                                </div>
                ...
                @else
                    <div class="no-results bg-white rounded-3xl p-12 text-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-800 mb-2 font-['IRANSansWebFaNum']">مقاله‌ای با این برچسب یافت نشد</h3>
                        <p class="text-gray-600 font-['IRANSansWebFaNum']">در حال حاضر مقاله‌ای با برچسب "{{ $tag->name }}" وجود ندارد.</p>
