...
# --- CPU Configuration for Training ---
print("Configuring CPU for training...")
...
# Disable mixed precision for CPU training (use float32)
keras.mixed_precision.set_global_policy('float32')
print("CPU configuration completed. Using float32 precision.")

# --- Settings ---
DATASET_DIR = 'captcha_dataset_enhanced'  # Updated to use enhanced clean dataset
MODEL_SAVE_PATH = 'persian_digit_model_cpu_enhanced.keras'  # Enhanced CPU model
IMG_WIDTH = 30
IMG_HEIGHT = 40
...
# --- Define CNN Model (CPU Optimized) ---
print("Building CNN model for CPU...")
input_shape = (IMG_HEIGHT, IMG_WIDTH, 1)  # 1 for grayscale

inputs = keras.layers.Input(shape=input_shape)

# CPU-optimized architecture with fewer parameters
...
plt.title('Training and Validation Accuracy (CPU)')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
...
plt.title('Training and Validation Loss (CPU)')
plt.xlabel('Epochs')
plt.ylabel('Loss')

plt.suptitle('Model Training History - CPU Version')
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig('training_history_cpu.png', dpi=300)
plt.show() 