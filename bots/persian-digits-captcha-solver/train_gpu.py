...
if gpus:
    ...
    print("GPU configuration completed. Using mixed precision training.")
else:
...
# --- Settings ---
DATASET_DIR = 'captcha_dataset'  # Updated to use enhanced clean dataset
MODEL_SAVE_PATH = 'persian_digit.keras'  # Enhanced GPU model
IMG_WIDTH = 30
IMG_HEIGHT = 40
...
# --- Define CNN Model (GPU Optimized) ---
print("Building CNN model for GPU...")
input_shape = (IMG_HEIGHT, IMG_WIDTH, 1)  # 1 for grayscale

inputs = keras.layers.Input(shape=input_shape)

# GPU-optimized architecture with more parameters for better performance
...
plt.title('Training and Validation Accuracy (GPU)')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
...
plt.suptitle('Model Training History - GPU Version')
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig('training_history_gpu.png', dpi=300)
plt.show() 